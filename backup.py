...
# Takes 2 positions, a prior position and a new position,
# returns if the position is a north, south, west or east move
def FindMoveDirection(priorPos, nextPos):
    x1 = priorPos[0]
    y1 = priorPos[1]
    x2 = nextPos[0]
    y2 = nextPos[1]
...
for timestep, obs in enumerate(observations):
    ...
    for posID, prevPos in enumerate(validPositions):
        prob = trellis[timestep-1][posID]

        if prob > maxProb:
            maxProb = prob
            mostLikelyPriorPositions = []
            # Add the position and position ID
            mostLikelyPriorPositions.append((posID,prevPos))
        elif prob == maxProb:
            mostLikelyPriorPositions.append((posID,prevPos))

    trellisRow = []
    for nextPosID, nextPos in enumerate(validPositions):
        if nextPosID != 9:
            continue
        #   b. Calculate a temporary set of probabilities "mostLikelyPriorPosProbs" using trellis[nextPosID,timestep] ← trellis[k, timestep - 1] * Tm_ki * Em_ij
        ...
        nextPositionProbs = []
        for mostLikelyPriorPosID, priorPos in enumerate(mostLikelyPriorPositions):
            priorPosID = 0
            for ID, pos in validPositions:
                if pos == priorPos:
                    priorPosID = ID
                    break

            prob = trellis[timestep-1][priorPosID] * Tm[priorPosID][nextPosID] * Em[nextPosID][timestep]
        ...
        #     priorPos = mostLikelyPriorPositions[posKID]
        #     print("Prior position:", priorPos)
        #     moveDir = FindMoveDirection(priorPos[1], nextPos)
        #     print(moveDir)
        #     moveDirectionsByKID[moveDir] = posKID

        # print("Next position:", (i, nextPos))
        # finalProb = 0
        # if moveDirectionsByKID.get("north"):
        #     finalProb = 
